...
const progressToast = readFileSync("src/components/VectorizationProgressToast.tsx", "utf8");
...
assert(desktopLayout.includes("VectorizationProgressToast"), "DesktopLayout must mount the extracted cross-tab background progress toast.");
assert(progressToast.includes("탭을 이동해도 이 작업은 백그라운드에서 계속 실행됩니다."), "VectorizationProgressToast must explain that background work survives tab changes.");
