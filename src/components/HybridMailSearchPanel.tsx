...
export function HybridMailSearchPanel({
  ...
  return (
    <section className="bg-surface rounded-2xl border border-surface-variant p-4 flex flex-col gap-4 min-h-0 overflow-hidden">
      <form onSubmit={onSubmit} className="flex flex-col gap-3">
        ...
        <div className="flex flex-wrap gap-2">
          {metadataExampleChips.map((chip) => (
            <button
              ...
              className="rounded-full border border-surface-variant bg-white px-3 py-1.5 text-[11px] font-semibold text-text-secondary transition-all hover:border-primary/30 hover:bg-primary-container/30 hover:text-text-primary disabled:opacity-50"
            >
              {chip.label}
            </button>
          ))}
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between gap-2">
            ...
            <button type="button" onClick={onLoadGmailLabels} disabled={!canUseGmail || searching || gmailLabelsLoading} className="rounded-full border border-surface-variant bg-white px-3 py-1.5 text-[11px] font-semibold text-text-secondary hover:border-primary/30 hover:bg-primary-container/25 hover:text-text-primary disabled:opacity-50">
              {gmailLabelsLoading ? "불러오는 중" : "라벨 불러오기"}
            </button>
          </div>
          <input type="text" value={labelSearch} onChange={(event) => onLabelSearchChange(event.target.value)} disabled={!canUseGmail || searching || gmailLabels.length === 0} placeholder="라벨 이름 검색" className="w-full bg-white border border-surface-variant rounded-full px-4 py-2.5 text-xs text-text-primary focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary disabled:opacity-50 transition-all placeholder:text-text-secondary/55" />
          <div className="max-h-24 overflow-y-auto flex flex-wrap gap-2 pr-1">
            {!isGwsAuthenticated ? (
              <p className="text-[11px] text-text-secondary">Google Workspace 인증이 필요합니다.</p>
            ) : gmailLabelsLoading ? (
              <p className="text-[11px] text-text-secondary">라벨을 불러오는 중입니다.</p>
            ) : gmailLabels.length === 0 ? (
              <p className="text-[11px] text-text-secondary">라벨 불러오기를 눌러 선택할 태그를 가져오세요.</p>
            ) : filteredLabels.length === 0 ? (
              <p className="text-[11px] text-text-secondary">일치하는 라벨이 없습니다.</p>
            ) : (
      ...
      <div className="rounded-2xl border border-surface-variant bg-white p-3 text-[11px] text-text-secondary">
        <div className="flex items-center justify-between gap-3">
        ...
        <p className="mt-2 leading-relaxed">검색된 메일은 오른쪽 목록에서 넓게 확인하고, 필요한 메일만 선택해 벡터화하세요.</p>
      </div>
